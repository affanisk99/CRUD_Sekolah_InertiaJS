							...
							<form class="space-y-8 p-2 divide-y divide-gray-200" @submit.prevent="submit">
										...
										<div class="space-y-6 sm:space-y-5">
											...
											<div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
												<label for="email" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
													...
													<button v-if="!student.deleted_at" class="float-left text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete student</button>
												</label>
												<div class="mt-1 sm:mt-0 sm:col-span-2">
												</div>
											</div>
								...
								<div class="pt-5">
									<div class="flex justify-end">
										<inertia-link :href="route('student.index')" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
											Cancel
										</inertia-link>
...
export default {
	...
	methods: {
		submit() {
		  this.$inertia.put(this.route('student.update', this.student.id), this.form, {
		...
		destroy() {
	      if (confirm('Are you sure you want to delete this student?')) {
	        this.$inertia.delete(this.route('student.destroy', this.student.id))
	      }
	    },
	},
}
