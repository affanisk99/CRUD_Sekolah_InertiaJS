            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            	<div class="pb-5 border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
					...
					<div class="flex mt-3 sm:mt-0 sm:ml-4">
						...
						<inertia-link :href="route('student.create')" class="inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
							Create
						</inertia-link>
					</div>
              	</div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                              <tr v-for="student in students.data" :key="student.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                 <td class="text-sm font-medium text-gray-700 whitespace-nowrap">
                                  <inertia-link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="route('student.edit', student.id)">
                                    {{ student.name }}
                                  </inertia-link>
                                </td>
                                <td class="text-sm font-medium text-gray-700 whitespace-nowrap">
                                 ...
                                 <td class="text-sm font-medium text-gray-700 whitespace-nowrap">
                                  <inertia-link class="flex items-center px-4" :href="route('student.edit', student.id)" tabindex="-1">
	...
	watch: {
      form: {
        handler: throttle(function() {
          ...
          this.$inertia.replace(this.route('student.index', Object.keys(query).length ? query : { remember: 'forget' }))
        }, 20),
        deep: true,
      }
	},
